...
import org.jrubyparser.runtime.encoding.EncodingService;
import org.jrubyparser.util.SafeDoubleParser;
...
public class RubyLexer extends LexingCommon {
    ...
    protected void setEncoding(ByteList name) {
        Encoding newEncoding = new EncodingService().loadEncoding(name);

        if (newEncoding == null) new RuntimeException("unknown encoding name: " + name.toString());
        if (!newEncoding.isAsciiCompatible()) throw new RuntimeException(name.toString() + " is not ASCII compatible");
